...
import time
from utils import *
from shape.vcamera import *
from shape.sphere import *
from shape.object3D import *
...
class Viewer:
    ...
    def render_rgb_viewport(self):
        ...
        for drawable in self.drawables:
            ...
            if self.selected_object == drawable and self.scale_changed:
                scale_matrix = scale(self.scale_factor) * scale(
                    1 / self.prev_scale_factor)  # to scale back before applying new scale
            ...
            # print(self.cameraPos, self.cameraFront, self.cameraUp)
            view = self.trackball.view_matrix()  ## modify here
            # todo
    ...
    def save_rgb(self, save_path):
        ...
        rgb_image = np.frombuffer(pixels, dtype=np.uint8).reshape(
    ...
    def save_depth(self, save_path):
        ...
        pixels = GL.glReadPixels(win_pos_width, 0, self.depth_view_width, self.depth_view_height, GL.GL_RGB,
                                 GL.GL_SHORT)  # return linear depth, not raw depth value
    ...
    def multi_cam(self):
        ...
        for i in range(self.num_vcameras):
            ...
            vcamera.projection = glm.perspective(glm.radians(self.fov), self.rgb_view_width / self.rgb_view_height, 0.1,
                                                 100.0)
    ...
    def load_scene(self):
        ...
        self.selected_scene = Scene(self.depth_texture_shader, obj_file)
        ...
        self.add(model)
        # this function just for add model
    ...
    def imgui_menu(self):
        ...
        # Adjust RGB
        self.bg_changed, self.bg_colors = imgui.input_float3('BG Color', self.bg_colors[0], self.bg_colors[1],
                                                             self.bg_colors[2], format='%.2f')
        ...
        imgui.set_next_item_width(100)
        self.scale_changed, self.scale_factor = imgui.input_float("Scale factor", self.scale_factor, step=0.1,
                                                                  format="%.2f")
        ...
        self.lightPos_changed, self.lightPos = imgui.input_float3('Position', self.lightPos[0], self.lightPos[1],
                                                                  self.lightPos[2], format='%.2f')

        # Add light color slider
        imgui.set_next_item_width(imgui.get_window_width() // 1.5)
        self.lightColor_changed, self.lightColor = imgui.input_float3('Color', self.lightColor[0], self.lightColor[1],
                                                                      self.lightColor[2], format='%.2f')
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.win):
            ...
            for drawable in self.drawables:
                ...
                if self.selected_object == drawable and self.scale_changed:
                    scale_matrix = scale(self.scale_factor) * scale(
                        1 / self.prev_scale_factor)  # to scale back before apply new scale
                    self.prev_scale_factor = self.scale_factor
                    drawable.update_attribute('model_matrix', scale_matrix)
                    # drawable.model = scale_matrix * drawable.model
